...
module camera_vga_top(
    ...
    always @(posedge clk) begin
        if (rst_clk)              det_update_toggle <= 1'b0;
    ...
    wire in_box_x = (vga_x >= box_x0) && (vga_x <= box_x1);
    wire in_box_y = (vga_y >= box_y0) && (vga_y <= box_y1);

    wire on_top    = in_box_x && (vga_y >= box_y0) && (vga_y <= box_y0 + 10'd1);
    wire on_bottom = in_box_x && (vga_y >= box_y1 - 10'd1) && (vga_y <= box_y1);
    wire on_left   = in_box_y && (vga_x >= box_x0) && (vga_x <= box_x0 + 10'd1);
    wire on_right  = in_box_y && (vga_x >= box_x1 - 10'd1) && (vga_x <= box_x1);
    ...
    // VGA pixel output
    //=======================================================================
    assign vga_red   = vga_active ? (on_box ? 4'h0 : filtered_pixel[11:8]) : 4'h0;
    assign vga_green = vga_active ? (on_box ? 4'hF : filtered_pixel[7:4])  : 4'h0;
    assign vga_blue  = vga_active ? (on_box ? 4'h0 : filtered_pixel[3:0])  : 4'h0;
