// detector_top: streaming face detector with bounding-box regression.
//
// One scan per camera frame, single scale (DILATE=3, 72x72 patch coverage).
// At every (scan_x, scan_y) the CNN produces five FC outputs:
//
//   fc_conf : confidence logit; > THRESHOLD selects the patch as a candidate
//   fc_x0   : face top-left X inside the 24x24 patch  (pixels << BBOX_SHIFT)
//   fc_y0   : face top-left Y inside the 24x24 patch
//   fc_w    : face width  inside the 24x24 patch
//   fc_h    : face height inside the 24x24 patch
//
// The patch with the highest fc_conf wins. Its (x0, y0, w, h) is dequantized
// by right-shifting BBOX_SHIFT bits, clamped to the valid patch range, scaled
// up by DILATE, and offset by (scan_x, scan_y) -> 160x120 frame coordinates.
// The result is reported as (face_x, face_y, face_w, face_h).
//
...
module detector_top #(
    ...
    // Right-shift count that dequantizes the FC bbox outputs into patch
    // pixels. Equals (W_FC_SCALE_EXP + ACT3_SCALE_EXP) from scales.vh;
    // export_weights.py prints the right number to use here.
    parameter integer BBOX_SHIFT = 9,
    // Confidence threshold on the RAW int32 logit. Sigmoid > 0.5 corresponds
    // to logit > 0, so THRESHOLD=0 means "accept any patch the network thinks
    // is a face". Increase for stricter detection.
    ...
    // FC widths sized for 5 outputs: 720 weights, 5 biases.
    wire [9:0]        fc_w_addr;
    ...
    // FC ROMs: 5*144=720 weights, 5 biases
    ...
    // -------------------------------------------------------------------
    // Dequantize the bbox outputs to patch pixels (0..23 for x/y, 1..24 for w/h)
    // -------------------------------------------------------------------
    ...
    // Patch pixel -> 160x120 frame pixel via *DILATE then offset by scan corner.
